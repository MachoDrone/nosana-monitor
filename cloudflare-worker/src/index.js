  ...
  const rows = hosts
    .map(
      ...
      <tr data-host="${name}" data-node="${h.nodeAddress || ''}" data-n="${h.n}" data-state="${h.state || ''}" data-q="${h.q}" data-seen="${h.seen}">
        <td style="padding:0 2px">${isDown(h.n, h.seen) ? '<span style="color:#888">?</span>' : tierIndicator(h.tier)}</td>
...
<body>
  ...
  ${
    ...
    <thead>
      <tr>
        <th data-col="tier" data-type="string" style="padding:0 2px"><div>Status</div></th>
