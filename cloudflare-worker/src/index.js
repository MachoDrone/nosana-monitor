  ...
  const rows = hosts
    .map(
      ...
      <tr data-host="${name}" data-node="${h.nodeAddress || ''}" data-n="${h.n}" data-state="${h.state || ''}" data-q="${h.q}" data-seen="${h.seen}">
        <td style="padding:0;width:1px;max-width:1px;overflow:hidden">${isDown(h.n, h.seen) ? '<span style="color:#888">?</span>' : tierIndicator(h.tier)}</td>
...
<body>
  ...
  ${
    ...
    <thead>
      <tr>
        <th data-col="tier" data-type="string" style="width:1px;max-width:1px;padding:0"><div>Status</div></th>
